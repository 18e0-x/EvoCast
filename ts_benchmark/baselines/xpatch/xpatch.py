...
class xPatch(DeepForecastingModelBase):
    ...
    def _post_process(self, output, target):
        cache_key = (self.config.horizon, output.device, output.dtype)
        if cache_key not in self._ratio_cache:
            ratio = [
                -1 * math.atan(i + 1) + math.pi / 4 + 1
                for i in range(self.config.horizon)
            ]
            self._ratio_cache[cache_key] = torch.tensor(
                ratio, dtype=output.dtype, device=output.device,
            ).unsqueeze(-1)

        ratio = self._ratio_cache[cache_key]
        output = output * ratio
        target = target * ratio
