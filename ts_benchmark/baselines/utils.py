...
# Map internal lowercase freq codes to pandas-compatible frequency strings.
# Python 3.14 pandas rejects uppercase single-letter codes (H, T, S, M).
# Valid: "h", "min", "s", "D", "W", "MS", "B".
_PANDAS_FREQ = {"h": "h", "t": "min", "s": "s", "d": "D", "w": "W", "m": "MS", "b": "B"}
...
def adjust_learning_rate(optimizer, epoch, args):
    ...
    if epoch in lr_adjust.keys():
        ...
        for param_group in optimizer.param_groups:
            param_group["lr"] = lr
        if os.environ.get("TFB_VERBOSE"): print("Updating learning rate to {}".format(lr))
...
def forecasting_data_provider(data, config, timeenc, batch_size, shuffle, drop_last):
    freq = getattr(config, "freq", "h")
    pandas_freq = getattr(config, "pandas_freq", None) or _PANDAS_FREQ.get(freq, freq)
...
class DatasetForTransformer:
    ...
    def __read_data__(self):
        self.dataset = _sanitize_transformer_dataset(self.dataset)
        if isinstance(self.dataset.index, pd.DatetimeIndex):
            stamp_series = pd.Series(self.dataset.index, name="date")
        else:
            stamp_series = pd.Series(
                pd.date_range("2000-01-01", periods=len(self.dataset), freq=self.pandas_freq),
                name="date",
            )
        df_stamp = stamp_series.to_numpy()[None, :]
        # decompose_time (timeenc=0) expects lowercase internal codes; time_features
        # (timeenc=1) expects pandas-compatible frequency strings (e.g. "min", "H").
        freq = self.pandas_freq if self.timeenc == 1 else self.freq
        data_stamp = get_time_mark(df_stamp, self.timeenc, freq)[0]
...
def _sanitize_transformer_dataset(dataset: pd.DataFrame) -> pd.DataFrame:
    ...
    for col in frame.columns:
        ...
        if pd.api.types.is_numeric_dtype(series):
            numeric_columns[col] = series.astype(np.float32)
            continue
        ...
        non_empty_mask = normalized.notna()
        if non_empty_mask.any() and converted[non_empty_mask].notna().all():
            numeric_columns[col] = converted.astype(np.float32)

    if not numeric_columns:
        raise ValueError("Transformer dataset has no forecasting-safe numeric columns after sanitization.")

    sanitized = pd.DataFrame(numeric_columns, index=frame.index)
    return sanitized
