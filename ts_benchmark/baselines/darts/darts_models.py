...
# Training / infrastructure params that should NEVER be passed to darts model
# constructors.  These are consumed by the TFB training loop or the darts
# adapter itself; passing them to darts models causes TypeError for models
# that don't accept them (e.g. FFT rejects num_epochs, batch_size, etc.).
_DARTS_TRAINING_PARAM_DENYLIST = {
    "num_epochs",
    "epoch",
    "epochs",
    "batch_size",
...
class DartsConfig:
    ...
    def get_darts_class_params(self) -> dict:
        """Return params for the darts model constructor, excluding training/infra keys."""
        ret = self.params.copy()
        ret.pop("norm")
        for training_key in _DARTS_TRAINING_PARAM_DENYLIST:
            ret.pop(training_key, None)
