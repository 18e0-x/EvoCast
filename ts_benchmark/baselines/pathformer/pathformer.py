...
class Pathformer(DeepForecastingModelBase):
    ...
    def _init_model(self):
        # ``num_nodes`` is Pathformer's structural name for the input-variable
        # dimension.  The shared forecasting base has already inferred
        # ``enc_in`` from the actual train dataframe at this point.  Keeping
        # the source default (21) here makes multivariate datasets whose
        # column count differs from 21 fail inside AMS.start_linear.
        self.config.num_nodes = int(self.config.enc_in)
