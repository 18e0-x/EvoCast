...
class RollingForecast(ForecastingStrategy):
    ...
    def _verbose_enabled() -> bool:
        return bool(os.environ.get("TFB_VERBOSE"))

    def _log_phase(self, series_name: str, phase: str, seconds: float) -> None:
        if self._verbose_enabled():
            print(
                f"[rolling_forecast] {series_name} {phase} took {seconds:.2f}s",
                flush=True,
            )

    @staticmethod
    def _log_rolling_progress(
        series_name: str,
        path: str,
        done: int,
        total: int,
        ...
        *,
        ...
        if total < 20 and not force:
            return
        if not force and done not in {1, total} and done % max(1, total // 10) != 0:
            return
        elapsed = time.time() - start_time
        rate = done / elapsed if elapsed > 0 else 0.0
        remaining = (total - done) / rate if rate > 0 else 0.0
        print(
            f"[rolling_forecast] {series_name} {path} {done}/{total} "
    ...
    def _eval_sample(
        ...
        self._log_phase(series_name, "fit", end_fit_time - start_fit_time)
        ...
        all_rolling_actual = []
        all_rolling_predict = []
        selected_index_list = list(itertools.islice(index_list, num_rollings))
        progress_start = time.time()
        self._log_rolling_progress(
            series_name,
            "sample_forecast",
            0,
            len(selected_index_list),
            progress_start,
            force=len(selected_index_list) >= 20,
        )
        for i, index in enumerate(selected_index_list):
            ...
            single_series_result = self.evaluator.evaluate(
                ...
                seasonality=self._get_seasonality(series, meta_info),
            )
    ...
    def _eval_batch(
        ...
        while predict_batch_maker.has_more_batches():
            ...
            done = min(batch_maker.current_sample_count, len(index_list))
            self._log_rolling_progress(
                series_name,
                "batch_forecast",
                done,
        ...
        self._log_phase(series_name, "batch_forecast", total_inference_time)

        start_eval_prep = time.time()
        ...
        self._log_phase(series_name, "eval_prep", time.time() - start_eval_prep)
        ...
        single_series_results = np.mean(np.stack(all_test_results), axis=0).tolist()
        self._log_phase(series_name, "metric_eval", time.time() - start_metric_eval)
    ...
    def field_names(self) -> List[str]:
        return self.evaluator.metric_names + [
            ...
            FieldNames.FORECAST_EXECUTION_PATH,
            FieldNames.LOG_INFO,
        ]
