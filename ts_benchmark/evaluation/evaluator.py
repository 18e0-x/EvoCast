...
class Evaluator:
    ...
    def evaluate(
        ...
        **kwargs,
        ...
        """
        return [self._call_metric(m, actual, predicted, scaler, hist_data, kwargs) for m in self.metric_funcs]

    @staticmethod
    def _call_metric(metric_func, actual, predicted, scaler, hist_data, kwargs):
        metric_kwargs = {"scaler": scaler, "hist_data": hist_data, **kwargs}
        bound_kwargs = getattr(metric_func, "keywords", {}) or {}
        for key in bound_kwargs:
            metric_kwargs.pop(key, None)
        return metric_func(actual, predicted, **metric_kwargs)
        ...
        for m in self.metric_funcs:
            try:
                evaluate_result.append(
                    self._call_metric(m, actual, predicted, scaler, hist_data, kwargs)
                )
            except Exception as e:
                evaluate_result.append(np.nan)
                metric_name = getattr(m, "__name__", None) or str(m)
