    ...
    def collect(self) -> Generator[pd.DataFrame, None, None]:
        ...
        for i, result in enumerate(
            tqdm.tqdm(
                ...
                disable=not bool(os.environ.get("TFB_VERBOSE")),
            )
        ):
            collector.add(
                _safe_execute(
...
def eval_model(
    ...
    result_list = []
    _quiet = not bool(os.environ.get("TFB_VERBOSE"))
    for series_name in tqdm.tqdm(
        series_list, desc=f"scheduling {model_factory.model_name}", disable=_quiet
    ):
