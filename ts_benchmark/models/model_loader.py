...
def _is_research_variant_module(module_name: str) -> bool:
    """True for generated research_variants modules."""
    name = str(module_name or "")
    return name.startswith("ts_benchmark.baselines.research_variants.")


def _remove_research_variant_bytecode(module_name: str) -> None:
...
def _import_module_fresh_if_research_variant(module_name: str) -> Any:
    """Import generated research variants from disk on every lookup.

    EvoCast rewrites variant files many times in the same Python process
    during repair loops.  Normal import caching would otherwise keep the
    first loaded class alive and make smoke tests diagnose stale code.

    Workspace variants are loaded only through model_config["variant_path"].
    Legacy research_variants modules still use normal import cache refresh.
    """
    ...
    _remove_research_variant_bytecode(module_name)
...
def _import_attribute(attr_path: str) -> Any:
    ...
    package = _import_module_fresh_if_research_variant(package_name)
...
def get_model_info(model_config: Dict) -> Union[Dict, Callable]:
    ...
    """
    variant_path = str(model_config.get("variant_path") or "").strip()
    workspace_root = None
    if variant_path:
        from evocast.variant.workspace_loader import load_module_from_variant_path

        workspace_root = Path(variant_path).resolve().parent
        module = load_module_from_variant_path(variant_path, keep_workspace_on_path=True)
        model_info = getattr(module, "Model", None)
        if model_info is None:
            raise AttributeError(f"Workspace variant has no Model export: {variant_path}")
    else:
        raw_model_name = str(model_config.get("model_name") or "")
        if "evocast_workspace" in raw_model_name:
            raise ValueError(
                "evocast_workspace model_name is no longer supported; "
                "workspace variants must be loaded with model_config['variant_path']"
            )
        ...
        for model_name in model_name_candidates:
            try:
                ...
                break
        if model_info is None and last_error is not None:
            raise last_error
